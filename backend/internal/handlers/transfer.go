...
func SendTransfer(userRepo *repository.UserRepo, transferRepo *repository.TransferRepo, solClient solana.ClientInterface) http.HandlerFunc {
        return func(w http.ResponseWriter, r *http.Request) {
                ...
                // Enforce sufficient balance (amount includes the 1% fee).
                balance, err := transferRepo.ComputeBalance(r.Context(), userID)
                if err != nil {
                        middleware.JSONError(w, http.StatusInternalServerError, "could not verify balance")
                        return
                }
                if req.AmountUSDT > balance {
                        middleware.JSONError(w, http.StatusBadRequest, "insufficient_balance: not enough USDT to complete this transfer")
                        return
                }
