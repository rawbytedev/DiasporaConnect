...
// randomHex returns n random hex-encoded bytes (2*n characters).
...
// InitiateTransfer records the transfer and returns a globally-unique fake hash
// built from crypto/rand so restarts never produce duplicate hashes.
func (m *MockSolanaClient) InitiateTransfer(senderID, recipientID uint, netAmount, fees float64) (string, uint64, error) {
        ...
        txHash := fmt.Sprintf("mock_%s_s%d_r%d", randomHex(16), senderID, recipientID)
        nonce := uint64(0)
...
// If the hash is not in the in-memory map (e.g. after a server restart) we
// still succeed — the database is the authoritative source of truth for state.
func (m *MockSolanaClient) ClaimTransfer(hash string) error {
        ...
        if rec, ok := m.transferHistory[hash]; ok {
                rec.Status = "claimed"
