...
func (r *UserRepo) GetUserByID(context context.Context, userID uint) (*models.User, error) {
        ...
        err = dbTx.QueryRow(context, "SELECT id, phone_number, solana_pubkey, encrypted_priv_key, name, kyc_verified, state_version, created_at FROM users WHERE id = $1", userID).Scan(&user.ID, &user.PhoneNumber, &user.SolanaPubkey, &user.EncryptedPrivKey, &user.Name, &user.KYCVerified, &user.StateVersion, &user.CreatedAt)
        if err != nil {
                return nil, err
        }
        return &user, nil
...
func (r *UserRepo) GetUserByPhone(ctx context.Context, phone string) (*models.User, error) {
        ...
        // cache miss
        err = dbTx.QueryRow(ctx, "SELECT id, phone_number, solana_pubkey, encrypted_priv_key, name, kyc_verified, state_version, created_at FROM users WHERE phone_number = $1", phone).Scan(&user.ID, &user.PhoneNumber, &user.SolanaPubkey, &user.EncryptedPrivKey, &user.Name, &user.KYCVerified, &user.StateVersion, &user.CreatedAt)
        if err != nil {
                return nil, err
        }
...
// SetKYCVerified marks the user as KYC-verified. Also invalidates the cache.
func (r *UserRepo) SetKYCVerified(ctx context.Context, userID uint) error {
        _, err := r.db.GetPool().Exec(ctx,
                "UPDATE users SET kyc_verified = true WHERE id = $1", userID)
        if err != nil {
                return err
        }
        return r.InvalidateUser(userID)
...
// GetUserByIDWithKYC fetches a user by ID including the kyc_verified column.
// (Alias kept for clarity; GetUserByID now also returns KYCVerified.)
func (r *UserRepo) GetUserByIDWithKYC(ctx context.Context, userID uint) (*models.User, error) {
